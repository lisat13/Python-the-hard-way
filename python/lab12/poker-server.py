...
hands = poker.deal(3)
handsdelt = 0 # Vi trenger en variabel som holder styr på hvor mange hender er del ut
players = 0
board = []
hands = []
...
#     Data leses med recv(), så blir plassert i en kø, slik at den kan bli sendt gjennom socketen tilbake til klienten
			...
			if data:
				...
				if data == 'JOIN\n':  
					if players == 3:
						print >>sys.stderr, 'bordet er fullt'
						error = 'spillet er fullt'
						message_queues[s].put(error)
					else:
						board.append(s.getpeername())
						players += 1
						joined = 'Du er spiller nr %d' % players
				...
				if data == 'DEAL\n':
					if players == 3:
						...
						dealt = 'kort utdelt'
						message_queues[s].put(dealt)
					else:
						data = 'Trenger flere spillere'
